...
if (isset($_SESSION['uid'])) {
    ...
    else
    {
        ...
        if (mysqli_num_rows($verify_auth_res) == 1)
        {
            ...
            else
            {
                ...
                // Create template
                $read_sql = "SELECT * FROM socmeds WHERE username = ?;";
                // Create a prepared statement
                $read_stmt = mysqli_stmt_init($con);

                // Prepare the prepared statement
                if (!mysqli_stmt_prepare($read_stmt, $read_sql))
                {
                    echo "Read SQL statement failed.";
                }
                else
                {
                    mysqli_stmt_bind_param($read_stmt, "s", $uid);
                    mysqli_stmt_execute($read_stmt);

                    // Get the result
                    $read_res = mysqli_stmt_get_result($read_stmt);

                    if (mysqli_num_rows($read_res) == 1)
                    {

                        $row = mysqli_fetch_assoc($read_res);

                        $_SESSION['website'] = $website = $row['website'];
                        $_SESSION['github'] = $github = $row['github'];
                        $_SESSION['twitter'] = $twitter = $row['twitter'];
                        $_SESSION['instagram'] = $instagram = $row['instagram'];
                        $_SESSION['facebook'] = $facebook = $row['facebook'];
    ...
    // $verify_auth = "SELECT * FROM users WHERE username = '$uid'";
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex flex-column align-items-center text-center">
                                            ...
                                            <div class="mt-3">
                                                ...
                                                <?php if ($address == "-") { $address = ""; } echo "<p class="."text-muted font-size-sm".">" .htmlspecialchars($address). "</p>";?>
